...
function isWorkflowStatus(v: string): v is WorkflowStatus {
  return v === 'draft' || v === 'active' || v === 'paused';
}
...
export async function POST(req: Request): Promise<NextResponse> {
  try {
    ...
    if (!statusStr || !isWorkflowStatus(statusStr)) return json(400, { ok: false, error: 'invalid_status' });
    ...
    const wfOk = await workflowBelongsToWorkspace({ admin, workspaceId, workflowId });
    if (!wfOk) return json(404, { ok: false, error: 'workflow_not_found' });
