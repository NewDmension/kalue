...
export async function POST(req: Request): Promise<NextResponse> {
  try {
    ...
    // Stage debe pertenecer a un pipeline del workspace
    ...
    if (stErr) return json(500, { ok: false, error: 'db_error', detail: stErr.message });
    if (!st) return json(404, { ok: false, error: 'stage_not_found' });

    const { data: p, error: pErr } = await admin
      .from('pipelines')
      .select('id')
      .eq('id', st.pipeline_id)
    ...
    if (!updated) return json(500, { ok: false, error: 'update_failed' });

    return json(200, { ok: true, stage: updated as unknown as StageRow });
  } catch (e: unknown) {
