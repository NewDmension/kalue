...
export async function POST(req: Request): Promise<NextResponse> {
  try {
    ...
    if (!me || String(me.role) !== 'owner') return NextResponse.json({ error: 'Forbidden' }, { status: 403 });

    // MVP: borrar memberships primero (si no hay FK cascade)
    await supabaseAdmin.from('workspace_members').delete().eq('workspace_id', workspaceId);

    const { error: delWsErr } = await supabaseAdmin.from('workspaces').delete().eq('id', workspaceId);
    if (delWsErr) return NextResponse.json({ error: 'Failed to delete workspace', detail: delWsErr.message }, { status: 400 });
